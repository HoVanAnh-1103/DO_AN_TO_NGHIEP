    ...
    return (<Content style={{ margin: '24px 16px 0', overflow: 'initial' }}>
        ...
        >
            ...
            <Table dataSource={noti}>
                ...
                <Column title="Tiêu đề" dataIndex="type" key="type" render={(data) => {
                    if ('THEM_LOP' == data) return 'Xác nhận lớp học'
                }} />
                <Column title="Nội dung" dataIndex="content" key="content" render={(_, data: any) => {
                    if ('THEM_LOP' == data.type) return <>	Bạn đã được PM <Tag>{data.user.fullName}</Tag> thêm vào một lớp dạy  <Tag>{JSON.parse(data.content).className}</Tag> . Hãy xác nhạn lớp hoc và chọn lịch dạy.</>
                }} />
                    ...
                    render={(_: any, record: DataType) => (
                        <Space size="middle">
                            <Button type="primary" onClick={() => {
                                navigate('/xac-nhan-lop-hoc?classId=' + JSON.parse(record.content).classId)
                            }}>
                                Xác nhận lịch học
                            </Button>
